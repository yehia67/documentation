...
|<a name="neighboring-socket-address"></a>`--neighboring-socket-address` |`NEIGHBORING_SOCKET_ADDRESS`|Bind the TCP server socket to a port |string |15600|The default address binds the TCP server socket to every network interface. Change this option if your node has multiple IP addresses and you only want to bind to a specific one.|
