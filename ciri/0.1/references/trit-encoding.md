...
**Because IOTA uses [ternary](root://getting-started/0.1/basics/ternary.md)-based computing and our current processors understand only binary, you need to encode ternary data into bytes. The cIRI has five encoding options, which correspond to the number of trits that are stored in one byte. The more trits stored in a byte, the more efficient the storage. However, because trits need to be extracted from a byte before being used by the hashing algorithm, the more trits in a byte, the more processing power is needed.** 
