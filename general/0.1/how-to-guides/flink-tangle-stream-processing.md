...
If you run your own IRI, you have to [enable and to configure the ZMQ API.](https://docs.iota.org/docs/iri/0.1/references/iri-configuration-options)
...
This guide describes step for step the the example in MostUsedAddresses.scala. 
The repository wit this and other examples is available [here](https://github.com/iota-community/flink-tangle-examples).
...
We simply get the ZMQ data stream by a defined hostname and port. We could also filter on a specific topic.
Since we are only using the UnconfirmedTransactionMessage, we can also filter on [tx](root://iri/references/zmq-events.md).
After adding the source and getting the stream, we filter by the [protobuf descriptor](https://developers.google.com/protocol-buffers/docs/reference/cpp/google.protobuf.descriptor). 

This stream now contains only UnconfirmedTransactionMessages. 
...
Now we have our stream of unconfirmed transaction messages. We basically get every message our full-node receives.
We want to find out which addresses were used the most. That means, we only need the address and some counter.
For simplicity we count every address in a transaction as one. We could also say we only count the positive values.
If you want to do that, you have to apply a filter with value > 0.
...
We only keep the address and a counter in a tuple.
...
Partitioning can also be really interesting if you want to process a huge amount of data.
...
Sliding Windows are really helpful for this. An update interval of 30 seconds is fine for our use-case.
...
This is a simple function which reduces all elements to the one we really need. 
...
Simple as that. The other variant is an aggregation class. One example:
...
Therefore in our case the reduce function makes more sense. 
Aggregation functions are helpful when you have complex operations. 
You can find one more complex example with Bundle aggregations.
The BundleAggregation shows how you can combine incoming transaction into a Bundle and split them into UnconfirmedBundles
and ReattachedUnconfirmedBundles. This example is also a simplification and should not be used in production.

Next we want to aggregate all elements and want to find out which elements are the top ten. 
So basically: Which addresses have the most transactions. 
The timeWindowAll functions returns a timeWindow over all aggregations. 
Since we used a SlidingWindow on our partitions before, the time here is not important anymore. 
...
Our TimeWindow contains all elements in a tuple (ADDRESS, AMOUNT_OF_TRANSACTIONS).
