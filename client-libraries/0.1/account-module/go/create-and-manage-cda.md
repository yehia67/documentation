# Make payments to/from your account in Go

**In this guide, you create a conditional deposit addressTo make payments, both the sender and the receiver need to have a conditional deposit address (CDA). The sender needs an expired CDA that contains IOTA tokens, and the receiver needs an active CDA to send tokens to.**
...
3. If you want to test this sample code with free test tokens, [request some from the Devnet faucet](root://getting-started/0.1/tutorials/get-test-tokens.md)

4. After making sure that the CDA is still active, send a deposit to it
