...
2. In the `one-command-tangle` directory, execute the `docker-compose up` command
 ...
 :::info:
 Compass uses a pre-built [Merkle tree](root://iota-basics/0.1/concepts/the-tangle.md) (in the `layers` directory) with a depth of 20. This Merkle tree is large enough for Compass to send milestones for over a year at 30-second intervals.
 :::
