...
**Note:** The higher the value of the `depth` parameter, the more computations the IRI node must do. To restrict the value of the `depth` parameter, IRI nodes can change the [`MAX-DEPTH`](../references/iri-configuration-options.md#max-depth) configuration option.
